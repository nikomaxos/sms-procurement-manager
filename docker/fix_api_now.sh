...
ALGO = "HS256"
ACCESS_MIN = int(os.getenv("ACCESS_TOKEN_MINUTES", "360"))

oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/users/login")
pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")

def get_password_hash(p: str) -> str:
    return pwd_context.hash(p if p is None or len(p) <= 72 else p[:72])
...
def create_access_token(sub: str) -> str:
    exp = int(time.time()) + ACCESS_MIN*60
...
# ---------- routers: countries / networks / suppliers / connections ----------
...
# ---------- routers/offers_plus.py (GET list/detail, POST by_names, PATCH id) ----------
cat > "$ROUT/offers_plus.py" <<'PY'
from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy import text
...
# migrate DB once on startup
migrate()

# Routers
from app.routers import users, countries, networks, suppliers, connections, offers_plus
...
# ---------- api.Dockerfile with pinned deps ----------
...
# ---------- rebuild & start only API ----------
echo "🔁 Building & starting API..."
( cd "$DOCKER_DIR" && docker compose build api && docker compose up -d api )
...
curl -sS http://localhost:8010/openapi.json | grep -A1 '"/users/login"' || echo "Missing /users/login"

echo "🔐 Try login (admin/admin123). If user missing, will create now..."
...
TOKEN=$(curl -sS -X POST http://localhost:8010/users/login -H "Content-Type: application/x-www-form-urlencoded" -d "username=admin&password=admin123" | python3 -c 'import sys,json; import json as j; s=sys.stdin.read(); print((j.loads(s)["access_token"]) if s else "")' || true)
echo "Token length: ${#TOKEN}"

echo "📦 GET /offers (should 200, maybe []):"
