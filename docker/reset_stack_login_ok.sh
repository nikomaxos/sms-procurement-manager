...
mkdir -p "$API" "$CORE" "$MODELS" "$ROUT" "$WEB"
...
@router.get("/trends") def trends(d: str): return {"date": d, "series": []}
PY
...
# main.py with global CORS and startup seeding
cat > "$MAIN_PY" <<'PY'
...
# IMPORTANT: dockerfile key must be INSIDE build:
cat > "$COMPOSE" <<'YAML'
services:
  postgres:
    image: postgres:15
...
curl -s -i -X POST "http://${IP}:8010/users/login" -H "Origin: http://${IP}:5183" -H "Content-Type: application/json" --data '{"username":"admin","password":"admin123"}' | sed -n '1,80p'
