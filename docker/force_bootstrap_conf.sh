...
# 1) Write routers/conf.py (fresh & idempotent)
...
# 2) Create boot.py that *always* includes conf and tries to include your other routers
cat > "$API_DIR/boot.py" <<'PY'
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware

app = FastAPI(title="SMS Procurement Manager")

# Permissive CORS for LAN UI on :5183
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Best-effort include of your other routers without breaking if missing
...
@app.get("/")
def root():
    return {"message": "OK", "version": "boot", "routers": [r.prefix for r in app.router.routes if hasattr(r, 'path')]}
PY

# 3) Point main.py to boot.app to avoid stale includes
...
# 5) Verify OpenAPI has /conf/enums (and endpoint works with a token)
sleep 2
...
# Get token (uses existing /users/login from your users router; if absent, this will fail visibly)
