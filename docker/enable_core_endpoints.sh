...
########################################
# Core already present from prior steps: database.py, auth.py
# (We won't overwrite those now.)
########################################
...
def migrate_domain():
    stmts = [
        ...
        """,
        # offers_current (main table used by UI)
        """
        CREATE TABLE IF NOT EXISTS offers_current(
          ...
          sender_id_supported VARCHAR(256),  -- CSV string (multi-select)
        ...
        """,
        # parser templates/configs (simple WYSIWYG + JSON rules)
        """
        ...
        """,
        # generic config_kv exists from earlier conf/enums work
        """
        ...
        """,
        # trigger to auto-update updated_at on offers_current
        """
        DO $$ BEGIN
        ...
        DO $$ BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM pg_trigger WHERE tgname = 'offers_current_touch_updated_at'
          ) THEN
...
########################################
# Suppliers (and nested Connections)
########################################
...
def create_network(payload:dict=Body(...), _=Depends(get_current_user)):
    ...
    if country_name:
        with engine.begin() as c:
            cr=c.execute(text("SELECT id, mcc, mcc2, mcc3 FROM countries WHERE name=:n"),{"n":country_name}).mappings().first()
...
def update_network(network_id:int, payload:dict=Body(...), _=Depends(get_current_user)):
    # allow updating name, country by name, mnc; recompute mccmnc
    changes={}; params={"id":network_id}
    name = payload.get("name"); 
    if name is not None: changes["name"]=name
    mnc = payload.get("mnc"); 
    if mnc is not None: changes["mnc"]=mnc
    country_name = payload.get("country_name")
    cid=None; mcc=None
    ...
    # compute new mccmnc if possible
    if "mnc" in changes or "country_id" in changes:
        # Need mcc from country
        ...
        if mcc and nmnc:
            new_mccmnc = f"{mcc}-{nmnc}"
        changes["mccmnc"]=new_mccmnc
...
def list_offers(
    ...
    sql = """
      SELECT o.id, o.price, o.currency, o.price_effective_date, o.previous_price,
             ...
             o.eta_days, o.charge_model, o.is_exclusive, o.notes, o.updated_by,
             s.organization_name AS supplier_name,
             sc.connection_name AS connection_name,
             c.name AS country_name, n.name AS network_name, o.mccmnc
...
def add_offer(payload:dict=Body(...), _=Depends(get_current_user)):
    ...
    # inherit charge_model if missing
...
def trends(d: str = Query(..., description="YYYY-MM-DD"), _=Depends(get_current_user)):
    # Count offers on that date if created, updated, or price_effective_date is that date
    sql = """
    ...
    # Organize top-10 per route_type
...
app.include_router(countries_router.router,prefix="/countries",tags=["Countries"])
app.include_router(networks_router.router, prefix="/networks", tags=["Networks"])
...
# main.py delegates to boot
cat > "$API/main.py" <<'PY'
from app.boot import app
PY

########################################
# Rebuild & quick checks
########################################
cd "$DOCKER_DIR"
...
[ -z "$TOK" ] && { echo "❌ login failed"; exit 1; } || echo "✅ token ok"
