...
echo "🔧 Ensure API has CORS (for http://localhost:5183 and 127.0.0.1)…"
...
echo "🧱 Create Web build context (Nginx static)…"
mkdir -p "$WEB_PUB"

# Minimal index.html with explicit API base
...
# Robust main.js (shows errors visibly)
...
# Create a Dockerfile named *Dockerfile* inside /web (compose will look for this by default)
cat > "$WEB_DIR/Dockerfile" <<'DOCKER'
FROM nginx:alpine
COPY public /usr/share/nginx/html
DOCKER

echo "🧩 Write compose override with correct build contexts…"
mkdir -p "$DOCKER_DIR"
...
echo "🩺 CORS preflight (look for Access-Control-Allow-Origin):"
...
echo "🔐 Test login via curl (should return access_token):"
curl -sS -X POST http://localhost:8010/users/login \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=admin&password=admin123" | sed -e 's/.*/  &/'

echo
echo "✅ Open Web UI: http://localhost:5183 and try logging in (admin / admin123)."
