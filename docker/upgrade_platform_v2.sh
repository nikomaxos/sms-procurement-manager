...
def create_access_token(sub: str, ttl_sec: int = 3600):
...
""",
# networks (store name + country + mcc/mnc + mccmnc)
"""
...
""",
# offers (name-linked; no FK for simplicity & speed)
"""
...
# move per_delivered to connections if someone had older column on suppliers (keep safe)
"DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='supplier_connections' AND column_name='per_delivered') THEN ALTER TABLE supplier_connections ADD COLUMN per_delivered BOOLEAN DEFAULT FALSE; END IF; END $$;"
]
...
def migrate():
    with engine.begin() as conn:
        ...
        safe_alters = [
          ...
          "ALTER TABLE offers ALTER COLUMN is_exclusive SET DEFAULT FALSE",
...
def login(form: OAuth2PasswordRequestForm = Depends()):
    ...
    token = auth.create_access_token(username, 86400)
...
def put_enums(body: dict, user: str = Depends(get_current_user)):
    with SessionLocal() as db:
        db.execute(text("INSERT INTO conf_enums(id,data) VALUES(1,:d) ON CONFLICT (id) DO UPDATE SET data=:d"),
                   {"d": body})
        db.commit()
        return {"ok": True}
PY
...
def list_suppliers(q: str | None = None, limit: int = 50, offset: int = 0, user: str = Depends(get_current_user)):
    ...
    if q:
        ...
        params["q"] = f"%{q}%"
    sql += " ORDER BY organization_name LIMIT :lim OFFSET :off"
...
def rename_supplier(name: str, body: dict, user: str = Depends(get_current_user)):
    ...
    with SessionLocal() as db:
        r = db.execute(text("UPDATE suppliers SET organization_name=:new WHERE organization_name=:old RETURNING id"),
                       {"new": new, "old": name}).first()
        if not r: raise HTTPException(404, "Not found")
        db.commit()
        return {"id": r.id, "organization_name": new}

@router.delete("/suppliers/{name}")
def delete_supplier(name: str, user: str = Depends(get_current_user)):
    with SessionLocal() as db:
        db.execute(text("DELETE FROM suppliers WHERE organization_name=:n"), {"n": name})
        db.commit()
        return {"ok": True}
...
def list_connections(supplier_name: str, q: str | None = None, user: str = Depends(get_current_user)):
    with SessionLocal() as db:
        ...
        if q: sql += " AND (connection_name ILIKE :q OR COALESCE(username,'') ILIKE :q)"
        params={"sid": sid}; 
        if q: params["q"]=f"%{q}%"
        return [dict(r._mapping) for r in db.execute(text(sql), params).all()]
...
def create_connection(supplier_name: str, body: dict, user: str = Depends(get_current_user)):
    with SessionLocal() as db:
        ...
        """), {"sid": sid, "n": body.get("connection_name"), "u": body.get("username"),
               "k": body.get("kannel_smsc"), "pd": body.get("per_delivered"), "cm": body.get("charge_model")})
        db.commit()
        return {"ok": True}
...
def delete_connection(supplier_name: str, connection_name: str, user: str = Depends(get_current_user)):
    with SessionLocal() as db:
        sid = db.execute(text("SELECT id FROM suppliers WHERE organization_name=:n"), {"n": supplier_name}).scalar()
        if not sid: raise HTTPException(404, "Supplier not found")
        db.execute(text("DELETE FROM supplier_connections WHERE supplier_id=:sid AND connection_name=:n"),
                   {"sid": sid, "n": connection_name})
        db.commit()
        return {"ok": True}
PY
...
def create_country(body:dict, user:str=Depends(get_current_user)):
    ...
    with SessionLocal() as db:
        db.execute(text("INSERT INTO countries(name,mcc,mcc2,mcc3) VALUES(:n,:m,:m2,:m3) ON CONFLICT (name) DO UPDATE SET mcc=EXCLUDED.mcc, mcc2=EXCLUDED.mcc2, mcc3=EXCLUDED.mcc3"),
...
def _pick_mcc(row):
    # prefer mcc; if multiple exist, return list
    arr = [x for x in [row.mcc, row.mcc2, row.mcc3] if x]
    return arr
...
def list_networks(q:str|None=None, country:str|None=None, mccmnc:str|None=None, limit:int=50, offset:int=0, user:str=Depends(get_current_user)):
    ...
    sql+=" ORDER BY country_name,name LIMIT :l OFFSET :o"
    params["l"]=limit; params["o"]=offset
    with SessionLocal() as db:
        return [dict(r._mapping) for r in db.execute(text(sql), params).all()
...
def create_network(body:dict, user:str=Depends(get_current_user)):
    name=(body.get("name") or "").strip()
    if not name: raise HTTPException(400,"name required")
    ctry=(body.get("country_name") or "").strip() or body.get("country") or None
    mnc = (body.get("mnc") or "").strip() or None
    with SessionLocal() as db:
        chosen_mcc = body.get("mcc")
        if ctry:
            row = db.execute(text("SELECT name,mcc,mcc2,mcc3 FROM countries WHERE name ILIKE :n"), {"n": ctry}).first()
            if row:
                mccs = [m for m in [row.mcc,row.mcc2,row.mcc3] if m]
                if len(mccs) == 1 and not chosen_mcc:
                    chosen_mcc = mccs[0]
        if chosen_mcc and mnc:
            mm = f"{chosen_mcc}{mnc}"
        else:
            mm = body.get("mccmnc")
        db.execute(text("""
          INSERT INTO networks(name,country_name,mcc,mnc,mccmnc)
          VALUES(:n,:c,:mcc,:mnc,:mm)
          ON CONFLICT DO NOTHING
        """), {"n":name,"c":ctry,"mcc":chosen_mcc,"mnc":mnc,"mm":mm})
        db.commit()
        return {"name":name}
...
def update_network(name:str, body:dict, user:str=Depends(get_current_user)):
    sets=[]; params={"old":name}
    for k in ("name","country_name","mnc","mcc","mccmnc"):
        if k in body:
            sets.append(f"{k}=:{k}"); params[k]=body[k]
    if "mcc" in body or "mnc" in body:
        sets.append("mccmnc=COALESCE(:mccmnc, (CASE WHEN COALESCE(:mcc,'')<>'' AND COALESCE(:mnc,'')<>'' THEN :mcc||:mnc ELSE mccmnc END))")
        params.setdefault("mccmnc", None)
    if not sets: return {"ok":True}
    with SessionLocal() as db:
...
def where_from_filters(p):
    ...
    if q:
        qq=f"%{q}%"
        wh.append("(notes ILIKE :qq OR network_name ILIKE :qq OR mccmnc ILIKE :qq)")
        params["qq"]=qq
...
def trends(d: str | None = None, user: str = Depends(get_current_user)):
    # date filter: include offers with created/updated/effective = selected date
    if not d:
        d = date.today().isoformat()
    with SessionLocal() as db:
        sql = """
        WITH picked AS (
          SELECT COALESCE(route_type,'__UNKNOWN__') as rt, COALESCE(network_name,'__UNKNOWN__') as net
          FROM offers
        ...
        """
        rows = [dict(r._mapping) for r in db.execute(text(sql), {"d": d}).all()]
    out = {"date": d, "by_route_type": {}}
    for r in rows:
        key = "Unknown" if r["rt"] == "__UNKNOWN__" else r["rt"]
        out["by_route_type"].setdefault(key, [])
        out["by_route_type"][key].append({"network_name": r["net"] if r["net"]!="__UNKNOWN__" else "Unknown", "count": r["cnt"]})
    # trim to top 10 per route type
...
def sup(q:str|None=None, user:str=Depends(get_current_user)):
    return _list("SELECT organization_name FROM suppliers WHERE organization_name ILIKE :q ORDER BY organization_name LIMIT 20",
                 {"q": f"%{q or ''}%"})
...
def con(q:str|None=None, supplier_name:str|None=None, user:str=Depends(get_current_user)):
    if supplier_name:
        with SessionLocal() as db:
            ...
            if not sid: return []
            return [r[0] for r in db.execute(text("SELECT connection_name FROM supplier_connections WHERE supplier_id=:sid AND connection_name ILIKE :q ORDER BY 1 LIMIT 20"),
                                              {"sid":sid, "q": f"%{q or ''}%"}).all()]
    return _list("SELECT connection_name FROM supplier_connections WHERE connection_name ILIKE :q ORDER BY 1 LIMIT 20",
                 {"q": f"%{q or ''}%"})
...
def net(q:str|None=None, country_name:str|None=None, user:str=Depends(get_current_user)):
    if country_name:
        return _list("SELECT name FROM networks WHERE country_name ILIKE :c AND name ILIKE :q ORDER BY 1 LIMIT 20",
                     {"c": f"%{country_name}%", "q": f"%{q or ''}%"})
    return _list("SELECT name FROM networks WHERE name ILIKE :q ORDER BY 1 LIMIT 20", {"q": f"%{q or ''}%"})
PY

############################################
# 3) main.py (CORS + include routers + migrations)
############################################
cat > "$API/main.py" <<'PY'
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from app.migrations import migrate
from app.routers import users, suppliers, countries, networks, offers, conf, metrics, lookups

app = FastAPI(title="SMS Procurement Manager", version="0.2.0")
...
@app.get("/")
def root():
    return {"message":"OK","version":"0.2.0"}
PY

############################################
# 4) Web UI (index.html, main.css, main.js)
############################################
...
async function verifyToken(){
  ...
  try{ await authFetch('/users/me'); return true; }catch{ localStorage.removeItem(tokenKey); TOKEN=''; return false; }
}
function onEnter(selector, fn){
  document.querySelectorAll(selector).forEach(el=>el.addEventListener('keydown', e=>{
...
$('#loginBtn').onclick = async ()=>{
  try{
    ...
    TOKEN = j.access_token; localStorage.setItem(tokenKey, TOKEN);
    setUser($('#user').value || 'admin'); closeLogin(); render('trends');
...
async function viewTrends(){
  ...
  $('#filters').innerHTML = `
    <div class="card row">
      ...
      <button id="tGo" class="btn btn-blue">Search</button>
    </div>`;
  ...
  const go = async ()=>{
    ...
    let html = '';
    for(const rt of [...keys.filter(k=>k!=='Unknown'), ...keys.filter(k=>k==='Unknown')]){
      const arr=data.by_route_type[rt]||[];
      const max = Math.max(1, ...arr.map(x=>x.count));
      html += `<div class="card"><h3>${rt}</h3>
        <div class="barwrap">
        ...
        <div class="legend">${arr.map(x=>`<legend-badge>${x.network_name}</legend-badge>`).join('')}</div>
      </div>`;
    }
...
/* ====== Offers (search, create, list, bulk, paging) ====== */
let OffersPager = {limit:50, offset:0, total:0};
...
async function viewOffers(){
  ...
  <div class="card">
    <div class="row">
      ...
      <div><label>Exclusive</label>
        <select id="of_ex"><option value=""></option><option>true</option><option>false</option></select>
      </div>
    </div>
    <div class="row">
      <button id="ofSearch" class="btn btn-blue">Search</button>
      <select id="pageSize">
        <option>10</option><option selected>50</option><option>100</option><option>1000</option>
      </select>
      <div class="pager">
  ...
  <datalist id="dl_con"></datalist>
  <datalist id="dl_cty"></datalist>
  <datalist id="dl_net"></datalist>
  ...
  `;
  setLookups('#of_sup', '/lookup/suppliers');
  setLookups('#of_con', '/lookup/connections');
  setLookups('#of_country','/lookup/countries');
  setLookups('#of_net','/lookup/networks');

  onEnter('#filters input', ()=> loadOffers(true));
  $('#ofSearch').onclick = ()=> loadOffers(true);
  ...
  $('#selAllPage').onclick = ()=> selectAllOnPage(true);
  $('#selAllAll').onclick = async ()=> await selectAllAcross(true);
  $('#bulkBtn').onclick = bulkUpdateSelected;

  $('#nf_create').onclick = async ()=>{
    const body = {
      supplier_name: $('#nf_sup').value.trim(),
      connection_name: $('#nf_con').value.trim(),
      country_name: $('#nf_country').value.trim() || null,
      network_name: $('#nf_net').value.trim() || null,
      mccmnc: $('#nf_mm').value.trim() || null,
      price: parseFloat($('#nf_price').value),
      price_effective_date: $('#nf_eff').value || null,
      previous_price: $('#nf_prev').value ? parseFloat($('#nf_prev').value) : null,
      route_type: $('#nf_route').value || null,
      known_hops: $('#nf_hops').value || null,
      sender_id_supported: $('#nf_sid').value || null,
      registration_required: $('#nf_reg').value || null,
      eta_days: $('#nf_eta').value ? parseInt($('#nf_eta').value,10) : null,
      charge_model: $('#nf_charge').value || null,
      is_exclusive: $('#nf_ex').value==='' ? null : ($('#nf_ex').value==='true'),
      notes: $('#nf_notes').value || null,
  ...
  OffersPager.limit = parseInt($('#pageSize').value,10);
  OffersPager.offset = 0;
  await loadOffers(true);
}

async function setLookups(selector, path){
  const input = document.querySelector(selector);
  const dl = document.querySelector(input.getAttribute('list'));
  input.addEventListener('input', async ()=>{
    const q=input.value;
    const url = path + '?q=' + encodeURIComponent(q);
    try{
      const arr = await authFetch(url);
      dl.innerHTML = arr.map(v=>`<option value="${v}">`).join('');
    }catch{}
  });
}

let pageData = []; let selectedIds = new Set();

function filtersQS(){
  const p=new URLSearchParams();
  const map = { '#of_sup':'supplier_name', '#of_con':'connection_name', '#of_country':'country', '#of_net':'network_name',
                '#of_mm':'mccmnc', '#of_route':'route_type', '#of_hops':'known_hops', '#of_sid':'sender_id_supported', '#of_reg':'registration_required' };
  for(const sel in map){
    const v=(document.querySelector(sel).value||'').trim(); if(v) p.set(map[sel], v);
  }
...
async function loadOffers(reset=false){
  ...
  OffersPager.total = total; pageData = rows;
  ...
  document.querySelectorAll('.rowchk').forEach(c=> c.onchange = e=>{
    const id = parseInt(c.dataset.id,10);
    if(c.checked) selectedIds.add(id); else selectedIds.delete(id);
  });
  document.querySelectorAll('button[data-act="del"]').forEach(b=> b.onclick = async ()=>{
    if(!confirm('Delete?')) return;
    await authFetch('/offers/'+b.dataset.id, {method:'DELETE'}); loadOffers();
  });
...
async function selectAllAcross(){ 
  // naive: load up to 1000 and select ids
  const qs = filtersQS().replace(/limit=\d+/,'limit=1000').replace(/offset=\d+/,'offset=0');
  const all = await authFetch('/offers/?'+qs);
  selectedIds = new Set(all.map(x=>x.id)); loadOffers();
}
...
async function bulkUpdateSelected(){
  ...
  if(!field) return;
  let val = prompt('Value (true/false for is_exclusive; integer for eta_days; text for others):','');
  if(field==='is_exclusive'){ val = (val==='true'); }
  else if(field==='eta_days'){ val = val? parseInt(val,10) : null; }
  await authFetch('/offers/bulk', {method:'POST', body: JSON.stringify({ids: Array.from(selectedIds), set: {[field]: val}})});
  await loadOffers();
}
...
async function openOfferEditor(id){
  ...
  if(!o){ alert('Row not found in page'); return; }
  const html = `
  <div class="card">
    <h3>Edit Offer #${o.id}</h3>
    <div class="row">
  ...
  $('#e_save').onclick = async ()=>{
    const body = {
      supplier_name: $('#e_sup').value.trim(),
      connection_name: $('#e_con').value.trim(),
      country_name: $('#e_country').value.trim() || null,
      network_name: $('#e_net').value.trim() || null,
      mccmnc: $('#e_mm').value.trim() || null,
      price: parseFloat($('#e_price').value),
      price_effective_date: $('#e_eff').value || null,
      previous_price: $('#e_prev').value ? parseFloat($('#e_prev').value) : null,
      route_type: $('#e_route').value || null,
      known_hops: $('#e_hops').value || null,
      sender_id_supported: $('#e_sid').value || null,
      registration_required: $('#e_reg').value || null,
      eta_days: $('#e_eta').value ? parseInt($('#e_eta').value,10) : null,
      charge_model: $('#e_charge').value || null,
      is_exclusive: $('#e_ex').value==='' ? null : ($('#e_ex').value==='true'),
      notes: $('#e_notes').value || null,
      updated_by: $('#userName').textContent || 'admin'
    };
    await authFetch('/offers/'+o.id, {method:'PUT', body: JSON.stringify(body)});
    await loadOffers();
  };
}

/* ====== Suppliers (expand connections inline) ====== */
async function viewSuppliers(){
  navActivate('suppliers');
  $('#filters').innerHTML = `
    <div class="card row">
      <input id="sq" placeholder="Search supplier…"/>
      <button id="sf" class="btn btn-blue">Search</button>
      <input id="snew" placeholder="New supplier name"/>
      <button id="screate" class="btn btn-green">Create</button>
    </div>`;
  $('#sf').onclick = renderSuppliers;
  onEnter('#filters input', renderSuppliers);
  ...
  ${list.map(s=>`
    <details>
      <summary><b>${s.organization_name}</b> 
        <span class="actions">
          <button class="btn btn-yellow" data-act="rn" data-id="${s.id}" data-name="${s.organization_name}">Rename</button>
          <button class="btn btn-red" data-act="del" data-name="${s.organization_name}">Delete</button>
        </span>
      </summary>
...
async function loadConnectionsPanel(s){
  ...
  $(dst).innerHTML = `
    <table>
      <thead><tr><th>Name</th><th>Username</th><th>SMSc</th><th>Per Delivered</th><th>Charge</th><th></th></tr></thead>
      <tbody>
        ${list.map(c=>`
          <tr>
            ...
            <td class="actions">
              <button class="btn btn-yellow" data-act="edit" data-n="${c.connection_name}">Edit inline</button>
              <button class="btn btn-red" data-act="del" data-n="${c.connection_name}">Delete</button>
            </td>
          </tr>
          <tr><td colspan="6">
            <div class="details-row" id="cedit-${s.id}-${c.connection_name}" style="display:none">
              <div class="row">
  ...
  $('#cadd-'+s.id).onclick = async ()=>{
    const body = {
      connection_name: $('#cname-'+s.id).value.trim(),
      username: $('#cuser-'+s.id).value.trim() || null,
      kannel_smsc: $('#csmsc-'+s.id).value.trim() || null,
      per_delivered: $('#cpd-'+s.id).checked,
      charge_model: $('#ccm-'+s.id).value
    };
    if(!body.connection_name) return;
    await authFetch('/suppliers/'+encodeURIComponent(s.organization_name)+'/connections/', {method:'POST', body: JSON.stringify(body)});
    await loadConnectionsPanel(s);
  };
  // inline edit
  $(dst).querySelectorAll('button[data-act="edit"]').forEach(b=> b.onclick = ()=>{
    const panel = $('#cedit-'+s.id+'-'+b.dataset.n);
    panel.style.display = panel.style.display==='none' ? '' : 'none';
  });
  $(dst).querySelectorAll('button[data-act="del"]').forEach(b=> b.onclick = async ()=>{
    if(!confirm('Delete connection?')) return;
    await authFetch('/suppliers/'+encodeURIComponent(s.organization_name)+'/connections/'+encodeURIComponent(b.dataset.n), {method:'DELETE'});
    await loadConnectionsPanel(s);
  });
  $(dst).querySelectorAll('button[data-act="save"]').forEach(b=> b.onclick = async ()=>{
    const body = {
      connection_name: $('#en-'+s.id).value.trim(),
      username: $('#eu-'+s.id).value.trim() || null,
      kannel_smsc: $('#ek-'+s.id).value.trim() || null,
      per_delivered: $('#ep-'+s.id).checked,
      charge_model: $('#ec-'+s.id).value.trim() || null
    };
    await authFetch('/suppliers/'+encodeURIComponent(s.organization_name)+'/connections/'+encodeURIComponent(b.dataset.n), {method:'PUT', body: JSON.stringify(body)});
    await loadConnectionsPanel(s);
  });
}
...
async function viewConnections(){
  ...
  $('#filters').innerHTML = `
    <div class="card row">
      <label>Supplier</label><input id="sq" placeholder="Supplier name"/>
      <label>Search</label><input id="cq" placeholder="contains…"/>
      <button id="cf" class="btn btn-blue">Search</button>
    </div>`;
  onEnter('#filters input', renderConnections);
  $('#cf').onclick = renderConnections;
  await renderConnections();
}
async function renderConnections(){
  const s = $('#sq').value.trim();
  ...
  $('#view').innerHTML = `<div class="card">
    <table><thead><tr><th>Name</th><th>Username</th><th>SMSc</th><th>Per Delivered</th><th>Charge</th></tr></thead>
    <tbody>${arr.map(c=>`<tr><td>${c.connection_name}</td><td>${c.username||''}</td><td>${c.kannel_smsc||''}</td><td>${c.per_delivered?'Yes':'No'}</td><td>${c.charge_model||''}</td></tr>`).join('')}</tbody></table>
  </div>`;
}
...
async function viewCountries(){
  ...
  $('#filters').innerHTML = `
    <div class="card row">
      ...
      <label>MCC</label><input id="cMcc"/>
      <label>2nd MCC</label><input id="cMcc2"/>
      <label>3rd MCC</label><input id="cMcc3"/>
      <button id="cCreate" class="btn btn-green">Create</button>
    </div>`;
  onEnter('#filters input', renderCountries);
  $('#cF').onclick = renderCountries;
  $('#cCreate').onclick = async ()=>{
    const body={name:$('#cName').value.trim(), mcc:$('#cMcc').value.trim()||null, mcc2:$('#cMcc2').value.trim()||null, mcc3:$('#cMcc3').value.trim()||null};
    if(!body.name) return; await authFetch('/countries/',{method:'POST', body:JSON.stringify(body)}); $('#cName').value='';$('#cMcc').value='';$('#cMcc2').value='';$('#cMcc3').value=''; renderCountries();
  };
...
async function renderCountries(){
  ...
  $('#view').innerHTML = `<div class="card">
    <table><thead><tr><th>Name</th><th>MCC</th><th>2nd MCC</th><th>3rd MCC</th><th></th></tr></thead>
      <tbody>${list.map(c=>`
        <tr><td>${c.name}</td><td>${c.mcc||''}</td><td>${c.mcc2||''}</td><td>${c.mcc3||''}</td>
          <td class="actions"><button class="btn btn-yellow" data-act="edit" data-name="${c.name}">Edit</button>
              <button class="btn btn-red" data-act="del" data-name="${c.name}">Delete</button></td></tr>
        <tr><td colspan="5">
          <div class="details-row" id="ced-${c.name}" style="display:none">
            <div class="row">
              <label>Name</label><input id="e_name_${c.name}" value="${c.name}">
              ...
              <button class="btn btn-yellow" data-act="save" data-name="${c.name}">Save</button>
            </div>
          </div>
        </td></tr>
      `).join('')}</tbody>
...
async function viewNetworks(){
  ...
  $('#filters').innerHTML = `
    <div class="card row">
      <label>Search name</label><input id="nq">
      <label>Country</label><input id="nc" list="dl_cty">
      <label>MCCMNC</label><input id="nmm">
      <button id="nF" class="btn btn-blue">Search</button>
      <label>Name</label><input id="nn">
      <label>Country</label><input id="ncountry" list="dl_cty">
      <label>MNC</label><input id="nmnc">
      <button id="nCreate" class="btn btn-green">Create</button>
    </div>`;
  onEnter('#filters input', renderNetworks);
  ...
  $('#nCreate').onclick = async ()=>{
    ...
    if(body.country_name){
      // If country has multiple MCCs, ask which one
      try{
        ...
        if(arr && arr[0]){
          ...
          if(m.length>1){
            const pick = prompt(`Country ${arr[0].name} has multiple MCCs (${m.join(', ')}). Type the one to use:`, m[0]);
            if(pick) body.mcc = pick;
          } else if(m.length===1) {
            body.mcc = m[0];
          }
        }
    ...
    await authFetch('/networks/',{method:'POST', body: JSON.stringify(body)});
    $('#nn').value='';$('#ncountry').value='';$('#nmnc').value='';
    renderNetworks();
  };
...
async function renderNetworks(){
  ...
  $('#view').innerHTML = `<div class="card">
    <table><thead><tr><th>Name</th><th>Country</th><th>MCC</th><th>MNC</th><th>MCCMNC</th><th></th></tr></thead>
    <tbody>${arr.map(n=>`
      <tr><td>${n.name}</td><td>${n.country_name||''}</td><td>${n.mcc||''}</td><td>${n.mnc||''}</td><td>${n.mccmnc||''}</td>
      <td class="actions"><button class="btn btn-yellow" data-act="edit" data-name="${n.name}">Edit</button>
      <button class="btn btn-red" data-act="del" data-name="${n.name}">Delete</button></td></tr>
      <tr><td colspan="6">
        <div class="details-row" id="ned-${n.name}" style="display:none">
          <div class="row">
            <label>Name</label><input id="en_${n.name}" value="${n.name}">
            <label>Country</label><input id="ec_${n.name}" value="${n.country_name||''}" list="dl_cty">
            <label>MCC</label><input id="em_${n.name}" value="${n.mcc||''}" readonly>
            <label>MNC</label><input id="emn_${n.name}" value="${n.mnc||''}">
            <label>MCCMNC</label><input id="emm_${n.name}" value="${n.mccmnc||''}" readonly>
            <button class="btn btn-yellow" data-act="save" data-name="${n.name}">Save</button>
          </div>
        </div>
      </td></tr>
    `).join('')}</tbody></table>
  ...
  $('#view').querySelectorAll('button[data-act="save"]').forEach(b=> b.onclick = async ()=>{
    const nm=$('#en_'+b.dataset.name).value.trim();
    const c=$('#ec_'+b.dataset.name).value.trim();
    const mnc=$('#emn_'+b.dataset.name).value.trim();
    let mcc=$('#em_'+b.dataset.name).value.trim();
    if(c){
      try{
        const arr = await authFetch('/countries/?q='+encodeURIComponent(c));
        if(arr && arr[0]){
          const m = [arr[0].mcc, arr[0].mcc2, arr[0].mcc3].filter(Boolean);
          if(m.length>1 && !m.includes(mcc)){
            const pick = prompt(`Country ${arr[0].name} has multiple MCCs (${m.join(', ')}). Type the one to use:`, m[0]);
            if(pick) mcc = pick;
          } else if(m.length===1){ mcc = m[0]; }
        }
      }catch{}
    }
    const mm = mcc && mnc ? mcc + mnc : ($('#emm_'+b.dataset.name).value||null);
    await authFetch('/networks/by-name/'+encodeURIComponent(b.dataset.name),{method:'PUT',body:JSON.stringify({name:nm, country_name:c||null, mnc:mnc||null, mcc:mcc||null, mccmnc:mm||null})});
    renderNetworks();
  });
}

/* ====== Parsers (simple WYSIWYG-like) ====== */
async function viewParsers(){
  ...
  $('#view').innerHTML = `
    <div class="card">
      <div class="row"><input id="pname" placeholder="Template name"><input id="pdesc" placeholder="Description"><button id="pcreate" class="btn btn-green">Create</button></div>
    </div>
    <div id="plist"></div>`;
  $('#pcreate').onclick = async ()=>{
    const body={name:$('#pname').value.trim(), description:$('#pdesc').value.trim(), html:"<p>Describe your parser here</p>", active:false};
    if(!body.name) return; await authFetch('/parsers/',{method:'POST', body:JSON.stringify(body)}); await viewParsers();
  };
  const wrap = $('#plist');
  wrap.innerHTML = list.map(p=>`
    <div class="card">
      <h3>${p.name}</h3>
      <div contenteditable="true" id="html_${p.id}" style="min-height:120px;border:1px dashed #1f2937;padding:8px">${p.html||''}</div>
      <div class="row right">
        <label><input type="checkbox" id="act_${p.id}" ${p.active?'checked':''}> Active</label>
        <button class="btn btn-yellow" data-id="${p.id}" data-act="save">Save</button>
        <button class="btn btn-red" data-id="${p.id}" data-act="del">Delete</button>
      </div>
    </div>`).join('');
  wrap.querySelectorAll('button[data-act="save"]').forEach(b=> b.onclick = async ()=>{
    const id=b.dataset.id; const html = $('#html_'+id).innerHTML; const active = $('#act_'+id).checked;
    await authFetch('/parsers/'+id,{method:'PUT', body:JSON.stringify({html, active})}); alert('Saved');
  });
  wrap.querySelectorAll('button[data-act="del"]').forEach(b=> b.onclick = async ()=>{
    if(!confirm('Delete?')) return; await authFetch('/parsers/'+b.dataset.id,{method:'DELETE'}); await viewParsers();
  });
}

/* ====== Settings (non-JSON UI) ====== */
async function viewSettings(){
  navActivate('settings');
  const e = await authFetch('/conf/enums');
  const mk = (title, key, arr)=>`
    <div class="card">
      <h3>${title}</h3>
      <div class="row" id="wrap_${key}">
        ${arr.map((v,i)=>`<span class="legend"><input value="${v}" id="${key}_${i}"> <button class="btn btn-red small" data-k="${key}" data-i="${i}">X</button></span>`).join('')}
      </div>
      <div class="row"><input id="new_${key}" placeholder="Add value"><button class="btn btn-green" data-add="${key}">Add</button></div>
    </div>`;
  $('#filters').innerHTML = '';
  $('#view').innerHTML = mk('Route Type','route_type', e.route_type||[]) + mk('Known Hops','known_hops', e.known_hops||[]) + mk('Registration Required','registration_required', e.registration_required||[]) + `<div class="row right"><button id="saveEnums" class="btn btn-blue">Save All</button></div>`;
  $('#view').querySelectorAll('button[data-add]').forEach(b=> b.onclick = ()=>{
    const key=b.dataset.add; const val=$('#new_'+key).value.trim(); if(!val) return;
    const wrap = $('#wrap_'+key); const idx = wrap.querySelectorAll('input').length;
    wrap.insertAdjacentHTML('beforeend', `<span class="legend"><input value="${val}" id="${key}_${idx}"> <button class="btn btn-red small" data-k="${key}" data-i="${idx}">X</button></span>`);
    $('#new_'+key).value='';
  });
  $('#view').addEventListener('click', (e)=>{
    const t=e.target; if(t.matches('button[data-k]')){ t.parentElement.remove(); }
  });
  $('#saveEnums').onclick = async ()=>{
    const get = (key)=> Array.from($('#wrap_'+key).querySelectorAll('input')).map(i=>i.value.trim()).filter(Boolean);
    const body = {route_type:get('route_type'), known_hops:get('known_hops'), registration_required:get('registration_required')};
    await authFetch('/conf/enums',{method:'PUT', body:JSON.stringify(body)}); alert('Saved');
  };
}

/* ====== Parsers API shim ====== */
async function parsersApiShim(){
  // simple router backed by offers table for storage-like functionality
}
JS

############################################
# 5) Add minimal Parsers router (CRUD)
############################################
...
def create(body:dict, user:str=Depends(get_current_user)):
    with SessionLocal() as db:
        db.execute(text("INSERT INTO parser_templates(name,description,html,active) VALUES(:n,:d,:h,:a)"),
                   {"n":body.get("name"),"d":body.get("description"),"h":body.get("html"),"a":bool(body.get("active"))})
        db.commit(); return {"ok":True}
...
############################################
# 6) Wire parsers into main.py
############################################
python3 - <<'PY'
from pathlib import Path, re
p = Path("""" + "$API/main.py" + """)
s = p.read_text()
if "from app.routers import users, suppliers, countries, networks, offers, conf, metrics, lookups, parsers" not in s:
    s = s.replace("from app.routers import users, suppliers, countries, networks, offers, conf, metrics, lookups",
                  "from app.routers import users, suppliers, countries, networks, offers, conf, metrics, lookups, parsers")
if "app.include_router(parsers.router)" not in s:
    s = s.replace("app.include_router(metrics.router)", "app.include_router(metrics.router)\napp.include_router(parsers.router)")
p.write_text(s)
PY

############################################
# 7) Ensure api.Dockerfile exists & build
############################################
...
############################################
# 8) Compose: (re)build api+web and start
############################################
cd "$DOCK"
...
echo "🩺 Root:"; curl -sS http://localhost:8010/ ; echo
...
echo "🌐 Web: http://localhost:5183  (If remote browser, set API to http://<LAN-IP>:8010 via ⚙ API button)"
